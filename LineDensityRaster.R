LineDensityRaster <- function(listOfSpatialPointsDF){
  #' Standardise values
  #'      Arg:
  ...
  LineDensity <- raster(outRasterExtent, crs = projection(pathList[1]), vals=0, 
                        ncols=outRasterExtent[2] - outRasterExtent[1], #xmax - xmin
                        nrows=outRasterExtent[4] - outRasterExtent[3]) #ymax - ymin
  ...
  for(i in pathList){
    ...
    singleRaster <- raster(outRasterExtent, crs = projection(i),
                           ncols=outRasterExtent[2] - outRasterExtent[1],
                           nrows=outRasterExtent[4] - outRasterExtent[3])
